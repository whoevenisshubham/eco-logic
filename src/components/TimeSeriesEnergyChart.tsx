...
export const TimeSeriesEnergyChart: React.FC = () => {
    ...
    const renderChart = useCallback(() => {
        ...
        const xExtent = d3.extent(allData, (point) => point.elapsed);
        ...
        g.append('g')
            ...
            .call((axis) => axis.selectAll('.tick line').attr('stroke', 'rgba(26,58,92,0.5)').attr('stroke-dasharray', '4,4'));
        ...
        chartArea
            ...
            .attr('filter', 'drop-shadow(0 0 4px rgba(0,212,255,0.6))');
        ...
        if (showBoth && dataB.length > 0) {
            ...
            chartArea
                ...
                .attr('stroke-width', 1.5)
                .attr('d', lineGenerator)
                .attr('filter', 'drop-shadow(0 0 4px rgba(0,255,136,0.5))');
        }
        ...
        chartArea
            ...
            .data(spikes)
            .join('circle')
            .attr('cx', (point) => xScale(point.elapsed))
        ...
        const xAxis = g
            ...
            .call(d3.axisBottom(xScale).ticks(6).tickFormat((value) => `${Number(value).toFixed(0)}ms`));
        ...
        g.append('rect')
            ...
            .on('mousemove', (event) => {
                ...
                const elapsed = xScale.invert(mouseX);

                const closest = dataA.reduce((best, point) => (
                    Math.abs(point.elapsed - elapsed) < Math.abs(best.elapsed - elapsed) ? point : best
                ));
        ...
        legend.append('text').attr('x', 24).attr('y', 10).attr('fill', '#7ab8d4').attr('font-size', '10px').text('Primary');

        if (showBoth && dataB.length > 0) {
            legend.append('line').attr('x1', 0).attr('x2', 20).attr('stroke', '#00ff88').attr('stroke-width', 2).attr('y1', 22).attr('y2', 22);
            legend.append('text').attr('x', 24).attr('y', 26).attr('fill', '#7ab8d4').attr('font-size', '10px').text('Secondary');
        }
                ...
                {isAnalyzing && (
                    <div className="absolute inset-0 z-20 bg-cyber-bg/70 backdrop-blur-sm flex items-center justify-center">
                        <div className="w-2/3 max-w-sm space-y-2">
                            ...
                            <p className="text-center text-xs font-mono text-cyber-accent">Synthesizing time-series energy trace...</p>
                        </div>
                    </div>
                )}
